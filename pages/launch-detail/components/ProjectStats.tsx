...
const ProjectStats: React.FC<ProjectStatsProps> = ({
  ...
  if (pair.state !== 0) {
    return (
      <div className={cn("flex flex-col items-center gap-3 md:gap-8", className)}>
        {pair.state !== 0 && (
          <CountdownTimer
    ...
    <div className={cn("flex flex-col items-center gap-3 md:gap-8", className)}>
      <div className="flex flex-wrap items-center justify-center md:justify-end gap-x-3 md:gap-x-6 gap-y-2 md:gap-y-3 text-xs w-full">
        ...
        <div className="flex flex-col items-center gap-1 md:gap-1.5">
          ...
          <span className={cn("text-sm md:text-[15px] font-bold", 
        ...
        <div className="flex flex-col items-center gap-1 md:gap-1.5">
          ...
          <span className="text-sm md:text-[15px] font-bold">
            {DynamicFormatAmount({
              amount: pair?.launchedToken?.totalValueLockedUSD || 0,
              decimals: 2,
              beginWith: "$",
            })}
          </span>
